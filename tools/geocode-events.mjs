...
const BOX = { minLat: 35.85, maxLat: 36.45, minLng: -115.55, maxLng: -114.85 };
...
function queryFor(f) {
  ...
  if (/\d{2,}/.test(addr) && addr.length > 8) {
    return /\bNV\b|nevada|las vegas|henderson/i.test(addr) ? addr : `${addr}, Las Vegas, NV`;
  }
  const venue = String(f.Venue || "").trim();
  if (venue.length > 2) return `${venue}, Las Vegas, NV`;
  return null;
}
