      ...
      <div className="rounded-blob border border-ink/10 bg-white p-8 shadow-card">
        {status === "done" ? (
          <>
            ...
            <Link href="/" className="hover-pop mt-5 inline-block rounded-full bg-coral px-5 py-3 font-800 text-white shadow-pop">Back to events</Link>
          </>
        ) : (
          <>
            <p className="text-4xl">📭</p>
            ...
            <button onClick={unsubscribe} disabled={status === "sending" || !email}
              className="hover-pop mt-5 rounded-full bg-coral px-5 py-3 font-800 text-white shadow-pop disabled:opacity-50">
