...
function Unsub() {
  ...
  async function unsubscribe() {
    ...
    const res = await fetch("/api/unsubscribe", {
      ...
      body: JSON.stringify({ email, t, kind }),
    });
        ...
        {status === "done" ? (
          <>
            ...
            <p className="mt-2 text-ink/70">
              {kind === "reminders"
                ? <>{email} won&apos;t get event reminders anymore.</>
                : <>{email} won&apos;t get the weekly digest anymore. Changed your mind? You can re-subscribe anytime.</>}
            </p>
          ...
          <>
            ...
            <p className="mt-2 text-ink/70">
              {kind === "reminders"
                ? <>Stop event-reminder emails to <strong>{email || "your email"}</strong>?</>
                : <>Stop sending the weekly Vegas Kiddos digest to <strong>{email || "your email"}</strong>?</>}
            </p>
